...
# @Last modified time: 2017-06-18T20:24:57+08:00
...
def do_query(month, daystart, dayend, query_host):
    bingo = {}; raw_data = []
    hostlist = getHost('host')
    for host in hostlist:
        sigmaset = []
        for day in range(daystart, dayend, query_host):
            cmd2 = "echo \"select av_load from com_cnc_gdp_mdio_wsms_par.load where monitor=\'%s\' and m=%s and d=%s  order by collect_time;\" > tmp.sql"%(host, month, day)
...
def main(argv=None):
    try:
        opts, args = getopt.getopt(argv[1:], 'hf:t:',  ["auto", "help", "test="])
    ...
    if len(opts) == 3:
        ...
        while True:
            ...
            if day > monthrange:
                month += 1
                day_start = 1
            threads.append(threading.Thread(target=do_query,args=(month, daystart, dayend, query_host ,)))
